...
type DedupStore interface {
	...
	// Abort removes a failed request from dedup tracking so immediate retries are not suppressed.
	Abort(ctx context.Context, hash string)
}
...
func (s *RedisDedupStore) Abort(ctx context.Context, hash string) {
	if s == nil || s.client == nil {
		return
	}
	_ = s.client.Del(ctx, s.prefix+hash).Err()
}
...
func (s *MemoryDedupStore) Abort(_ context.Context, hash string) {
	if s == nil {
		return
	}
